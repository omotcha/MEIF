...
class ECIFP_Predictor:
    ...
    def _predict(self, f_pro, f_lig):
        ...
        :return:
        """
        ecifp = self._ecifp_helper.get_ecifp(f_pro, f_lig, float(6.0))
        ld = self._ecifp_helper.get_ligand_features_by_file(f_lig)
        data = ecifp + list(ld)
    ...
    def _predict_on_decoy_by_id_mol2(self, lid):
        ...
        self._ecifp_helper.cache_protein(f_pro)
    ...
    def multi_ligd_pred(self, test_name, model):
        ...
        for i in ids:
            ...
            for j in tqdm(os.listdir(f_ligs)):
                ...
                pk_ag = self._predict(f_prot, f_lig)
    ...
    def predict_on_core(self, model):
        ...
        for i in tqdm(ids):
            ...
            pk_ag = self._predict(f_prot, f_ligd)
            preds_ag.append(pk_ag)
            lig_names.append(i)
        result = pd.DataFrame({"#code": lig_names, "score": preds_ag}, columns=["#code", "score"])
        result.to_csv(os.path.join(tmp_dir, "ecifp.dat"), index=False)
    ...
    def predict_on_single_decoy_file(self, f_dec, model):
        ...
        if postfix in {"sdf", "mol2"}:
            ecifps = self._ecifp_helper.__getattribute__("get_decoys_ecifp_cached_" + postfix)(f_dec, float(6.0))
            ld = self._ecifp_helper.__getattribute__("get_ligand_features_by_decoy_" + postfix)(f_dec)
        else:
            return
        cols = self._ecifp_helper.get_possible_pl() + LIGAND_DESC
        ret = []
...
if __name__ == '__main__':
    ...
    # target_based_predict_modulator(6, ecifp_lightgbm)
