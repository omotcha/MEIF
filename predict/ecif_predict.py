...
class ECIF_Predictor:
    ...
    def _predict_wold_sdf(self, f_pro, f_lig):
        """
        predict on protein-ligand pairs not in pdbbind dataset, without ligand descriptors
        :param f_pro: protein file, .pdb
        :param f_lig: ligand file, .sdf
        :return:
        """
        ecif = self._ecif_helper.get_ecif(f_pro, f_lig, float(6.0))
        cols = self._ecif_helper.get_possible_pl()
        data_f = pd.DataFrame([ecif], columns=cols)
        return self._model.predict(data_f)[0]
    ...
    def _predict_wold_on_decoy_by_id_mol2(self, lid):
        """
        for CASF docking power analysis
        :param lid: ligand/protein id
        :return:
        ...
        result = pd.DataFrame({"#code": dec_names, "score": ret}, columns=["#code", "score"]) \
            .sort_values(by="score", ascending=False)
        result.loc[-1] = ["{}_ligand".format(lid), self._predict_wold_sdf(f_pro, f_lig)]
        result.to_csv(os.path.join(tmp_dir, "ecif_decoy_pred", "{}_score.dat".format(lid)), index=False)
        return
    ...
    def _predict_wold_on_decoy_by_id_sdf(self, lid):
        """
        for CASF docking power analysis, decoys in sdf format
        :param lid: ligand/protein id
        :return:
    ...
    # callables
    def predict_sdf(self, f_pro, f_lig):
        """
        predict on protein-ligand pairs not in pdbbind dataset
        :param f_pro: protein file, .pdb
        :param f_lig: ligand file, .sdf
    ...
    def predict_wold_sdf(self, f_pro, f_lig):
        """
        predict on protein-ligand pairs not in pdbbind dataset, without ligand descriptors
        :param f_pro: protein file, .pdb
        :param f_lig: ligand file, .sdf
    ...
    def predict_wold_on_core(self, model):
        """
        for CASF analysis, make predictions on CASF core set
        :param model: specific model generated by sklearn
        :return:
        ...
        for i in tqdm(ids):
            f_prot = os.path.join(casf_dir["core"], "{}\\{}_protein.pdb".format(i, i))
            f_ligd = os.path.join(casf_dir["core"], "{}\\{}_ligand.sdf".format(i, i))
            pk = self._predict_wold_sdf(f_prot, f_ligd)
            preds.append(pk)
            lig_names.append(i)
        result = pd.DataFrame({"#code": lig_names, "score": preds}, columns=["#code", "score"])
        result.to_csv(os.path.join(tmp_dir, "ecif.dat"), index=False)
    ...
    def predict_wold_on_decoy_by_id(self, lid, model):
        """
        for CASF docking power analysis
        :param lid: ligand/protein id
        :param model:
    ...
    def predict_wold_on_decoy_mol2(self, model):
        """
        for CASF docking power analysis
        :param model:
        :return:
        ...
        for lid in lids:
            try:
                self._predict_wold_on_decoy_by_id_mol2(lid)
            except Exception:
                err_lids.append(lid)
                continue
        end = time.perf_counter()
    ...
    def predict_wold_on_decoy_sdf(self, model):
        """
        for CASF docking power analysis
        :param model:
        :return:
        ...
        for lid in lids:
            try:
                self._predict_wold_on_decoy_by_id_sdf(lid)
            except Exception:
                err_lids.append(lid)
                continue
        end = time.perf_counter()
    ...
    def predict_wold_on_single_decoy_file(self, f_dec, model):
        """
        do prediction on single decoy file,
        for those fail in either predict_wold_on_decoy_mol2() or predict_wold_on_decoy_sdf()
        :param f_dec: decoy file, decoys in sdf/mol2 format
        :param model:
        :return:
        """
    ...
    def predict_wold_on_target_decoy_sdf(self, f_tar, f_dec):
        """
        for CASF screening power analysis
        :param f_tar: target file, in .pdb format
        :param f_dec: decoy file, in .sdf format
    ...
    def predict_wold_on_target_decoy_mol2(self, f_tar, f_dec):
        """
        for CASF screening power analysis
        :param f_tar: target file, in .pdb format
        :param f_dec: decoy file, in .mol2 format
...
def target_wold_based_predict_worker(targets, model):
    """
    ...
    for tid in targets:
        ...
        for f_dec in os.listdir(f_decs_base):
            try:
                target_predictor.predict_wold_on_target_decoy_sdf(f_pro, os.path.join(f_decs_base, f_dec))
            except Exception:
                err_ids.append(f_dec)
        if len(err_ids) > 0:
            with open(os.path.join(log_dir, "screening", "ecif", "{}.err".format(tid)), 'w') as f:
...
def target_wold_based_predict_modulator(n_workers=4, model=None):
    """
    for CASF screening power analysis, this function serves as a VS modulator
    :param n_workers:
    :param model:
    ...
    start = time.perf_counter()
    pool = multiprocessing.Pool(n_workers)
    pool.starmap(target_wold_based_predict_worker, zip(worker_tasks, [model] * n_workers))
    end = time.perf_counter()
    print('\n')
    print('run time: {} seconds'.format(round(end - start)))
...
def target_wold_based_on_single_decoy_file(f_dec_postfix, pred):
    """
    do prediction on single decoy file, for those fail in target_based VS
    :param f_dec_postfix: the postfix of decoy file in .mol2 format
    :param pred:
    ...
    def get_targets():
        ...
        return ret
    if pred is None:
        pred = ECIF_Predictor(ecif_wold)
    targets = get_targets()
    for tid in targets:
        f_pro = os.path.join(casf_dir["core"], tid, "{}_protein.pdb".format(tid))
        f_dec = os.path.join(casf_dir["decoys_screening"], tid, "{}_{}".format(tid, f_dec_postfix))
        pred.predict_wold_on_target_decoy_mol2(f_pro, f_dec)
